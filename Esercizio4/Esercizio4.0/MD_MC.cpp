...
int main()
{
  ...
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; istep++)
    {
      ...
      if(istep%10 == 0){
        //ConfXYZ(nconf);
				//Write actual configuration in XYZ format //Commented to avoid "filesystem full"!
        nconf += 1;
      }
    }
...
void Averages(int iblk) //Print results for current block
{
  ...
  */
	...
	Epot.open("output_epotG.dat",ios::app);
	Ekin.open("output_ekinG.dat",ios::app);
	Temp.open("output_tempG.dat",ios::app);
	Etot.open("output_etotG.dat",ios::app);
	Ptot.open("output_ptotG.dat",ios::app);
	...
	stima_pressure = blk_av[ip]/blk_norm; //Pressure
