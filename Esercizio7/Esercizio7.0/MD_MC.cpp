...
int main()
{
  ...
  int nconf = 1;
	int eq_steps = 10000;
	// Code to reach temperature equilibrium
	...
	// Reset block averages after reaching equilibrium
	//Reset(0);
...
//        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"! 
	...
	ofstream GrOutput;
	GrOutput.open("output_gr.dat", ios::app);

	for (int i = 0; i < nbins_histo; ++i) {
		double r = (i+0.5) * bin_size_histo; // Calculate the midpoint of the bin
		double gr_avg = gr_av[i]/nblk; // Average value of g(r)
		double gr_avg2= gr_av2[i]/nblk;
		double gr_err = Error(gr_avg,gr_avg2,nblk); // Statistical uncertainty of g(r)

		GrOutput << "\t" << r << "\t" << gr_avg << "\t" << gr_err << endl;
	}
...
{
	bin_size_histo=box / (2.0 * nbins_histo);
  double v = 0.0,vp=0.0, kin=0.0;
  double vij, Vtail, Ptail;
  double vijp;
  double dx, dy, dz, dr;
...
// distance i-j in pbc
      ...
      dz = Pbc(z[i] - z[j]);
			...
			//UPDATE OF HISTOGRAM OF g(r)
			
      dr = dx*dx + dy*dy + dz*dz;
      dr = sqrt(dr);
			
			if (dr < box / 2.0) {
					int bin = static_cast<int>(dr / bin_size_histo); //Determine the bin index
					gr[bin] += 2.0; //Increment the count for the bin
			}
			
      if(dr < rcut)
      {
				vijp= pow(1/dr,12) - 0.5*pow(1/dr,6);
...
{
	bin_size_histo=box / (2.0 * nbins_histo);
	double Vtail=0., Ptail=0.;
	ofstream Epot, Ekin, Etot, Temp, Ptot;
	const int wd=12;
	/*
	glob_av[iv] =0.;
	glob_av2[iv]=0.;
    ...
    */
	...
	Ptot.open("output_ptot.dat",ios::app);
	...
	stima_pressure = blk_av[ip]/blk_norm-Ptail; //Pressure
	glob_av[ip] += stima_pressure;
	glob_av2[ip] += stima_pressure*stima_pressure;
	err_pressure=Error(glob_av[ip],glob_av2[ip],iblk);
...
//Temperature
	...
	//Pressure
	Ptot << setw(wd) << iblk <<  setw(wd) << stima_pressure << setw(wd) << glob_av[ip]/(double)iblk << setw(wd) << err_pressure << endl;


	for (int i = 0; i < nbins_histo; ++i) {
		double dv = (4. / 3.) * M_PI * (pow((i + 1) * bin_size_histo, 3) - pow(i * bin_size_histo, 3)); // Volume of the shell
		double gr_avg = gr[i] / (rho * npart * dv * blk_norm); // Average value of g(r)
		gr_av[i]+=gr_avg;
		gr_av2[i]+=gr_avg*gr_avg;
	}
...
/*
 ...
 walker[ik] = kin; // Kinetic energy
 walker[it] = t; // Temperature
 walker[ie] = 4.0 * v + kin + Vtail;  // Total energy;
 walker[ip] = rho*t+(1./(3.*vol))*(48*vijp+Ptail);
